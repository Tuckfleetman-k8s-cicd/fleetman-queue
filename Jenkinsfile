pipeline {
    ...
    environment {
        ...
        ORGANIZATION_NAME = "Tuckfleetman-k8s-cicd"
    ...
    stages {
        ...
        stage('Update Deployment file') {
            environment {
                ...
                GIT_ORG_NAME = "Tuckfleetman-k8s-cicd"
            }
            steps {
                withCredentials([string(credentialsId: 'github', variable: 'GITHUB_TOKEN')]) {
                    sh '''
                        git config user.email "yanis.devops1@gmail.com"
                        git config user.name "YanisDevops"
